...
async function getUserById(id) {
    ...
    user.marketingPref = preferences.map(p=>{
...
async function createUser({ name, email, password, salutation, country, marketingPref }) {
  ...
  try {
    ...
    if (Array.isArray(marketingPref)) {
      for (const preference of marketingPref) {
...
async function updateUser(id, { name, email, salutation, country, marketingPref }) {
  if (!id || typeof id !== 'number') {
    throw new Error('Invalid user ID');
  }
